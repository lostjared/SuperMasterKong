...
struct SDL_Font *SDL_InitFont(const char *src) {
	...
	size_t status = fread((char*)&q, sizeof(int), 1, fptr);
	if(status <= 0) {
	...
	if(q == -100) {
		status = fread((char*)&mx, sizeof(int), 1, fptr);
		status = fread((char*)&my, sizeof(int), 1, fptr);
		status = fread((char*)&fnt->tcolor, sizeof(int), 1, fptr);
		...
		for(i = 0; i < fnt->mx; i++) {
			for(z = 0; z < fnt->my; z++) {
				status = fread((char*)&fnt->letters[p].fnt_ptr[i][z], sizeof(int), 1, fptr);
